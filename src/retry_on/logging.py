"""Module for logging utilities. Most of it was temporarily borrowed from the marvin project."""
...
def get_logger(
    ...
    Example:
        ...
        logger = get_logger("retry_on.test")
        logger.info("This is a test") # Output: retry_on.test: This is a test

        debug_logger = get_logger("try_on.debug")
        debug_logger.debug_kv("TITLE", "log message", "green")
        ```
    """
    parent_logger = logging.getLogger("retry_on")

    if name:
        if not name.startswith(f"{parent_logger.name}."):
